...
# means = [ablation_results[v]["mean"] for v in variants]
# stds = [ablation_results[v]["std"] for v in variants]

# plt.bar(variants, means, yerr=stds, capsize=5, color='skyblue')
# plt.ylabel("Average F1-macro")
# plt.title("Ablation Study: Preprocessing Variants")
...
ax.legend()
plt.tight_layout()
plt.savefig(f"outputs_pt/plot3_preprocessing_ablation.png", bbox_inches="tight")
plt.show()
